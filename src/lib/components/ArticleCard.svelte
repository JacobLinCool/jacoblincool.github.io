...
<div class="article-card w-full pb-4">
	<a id="link" href="/blog/post/{post.slug}">
		<div
		...
		>
			<div class="h-full flex flex-col md:flex-row justify-between">
				{#if cover}
					<div
						class="w-full md:h-full flex justify-center md:max-w-[14rem] lg:max-w-[16rem] xl:max-w-xs"
					>
						<div
					...
					<div class="h-full flex flex-col md:flex-row justify-between md:items-end">
						<div class="flex flex-row">
							<div class="flex flex-row items-center">
								<div class="flex flex-row items-center">
									<h2 class="ml-2 text-slate-700 font-bold text-lg md:text-2xl">
										{post.title}
									</h2>
								</div>
							</div>
						...
						<div class="flex flex-row">
							<div class="flex flex-row items-center mr-2">
								<p class="ml-2 text-slate-500">
									{new Date(post.date).toDateString()} · {post.readtime} read
								</p>
							</div>
						</div>
					</div>
					<hr class="border border-slate-300 mx-2 mb-2" />
